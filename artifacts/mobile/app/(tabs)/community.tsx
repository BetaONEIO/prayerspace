...
import { useCommunityGroups, communityGroupStore } from "@/lib/communityGroupStore";
    ...
    <SafeAreaView style={[styles.safeArea, { backgroundColor: colors.background }]} edges={["top"]} {...swipeHandlers}>
      ...
      <View style={styles.header}>
        ...
        <View style={styles.headerIcons}>
          ...
          {activeTab === "Groups" && isActiveCommunityAdmin && (
            <Pressable
              style={styles.iconBtn}
              onPress={() => {
                if (Platform.OS !== "web") void Haptics.selectionAsync();
                router.push({ pathname: "/create-group", params: { communityId: activeCommunity.id } } as never);
              }}
            >
      ...
      {activeTab === "Groups" ? (
        <CommunityGroupsContent communityId={activeCommunity.id} isAdmin={isActiveCommunityAdmin} />
      ) : (
        <AutoScrollView
...
function createCgStyles(colors: ReturnType<typeof useThemeColors>) {
...
function CommunityGroupsContent({ communityId, isAdmin }: { communityId: string; isAdmin: boolean }) {
  const colors = useThemeColors();
  const cgStyles = useMemo(() => createCgStyles(colors), [colors]);
  const router = useRouter();
  ...
  const handleJoin = useCallback((groupId: string) => {
    if (Platform.OS !== "web") void Haptics.notificationAsync(Haptics.NotificationFeedbackType.Success);
    communityGroupStore.join(groupId);
  }, []);

  const handleCreate = useCallback(() => {
    if (Platform.OS !== "web") void Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Light);
    router.push({ pathname: "/create-group", params: { communityId } } as never);
  }, [router, communityId]);

  return (
    <ScrollView
      contentContainerStyle={cgStyles.scrollContent}
      showsVerticalScrollIndicator={false}
    >
      <View style={cgStyles.contextCard}>
        <Text style={cgStyles.contextText}>
          Groups are focused prayer spaces within your community — deeper, more personal conversations with a trusted circle.
        </Text>
      </View>

      {isAdmin && (
        <Pressable style={cgStyles.createBtn} onPress={handleCreate}>
          <View style={{ width: 28, height: 28, borderRadius: 10, backgroundColor: colors.primaryForeground + "25", alignItems: "center", justifyContent: "center" }}>
            <Plus size={15} color={colors.primaryForeground} />
          </View>
          <Text style={cgStyles.createBtnText}>Create a group</Text>
      ...
      {joinedGroups.length > 0 && (
        <>
          <View style={cgStyles.sectionHeader}>
            <Users size={12} color={colors.mutedForeground} />
            <Text style={cgStyles.sectionTitle}>YOUR GROUPS</Text>
          </View>
          {joinedGroups.map((g) => (
            <Pressable key={g.id} style={cgStyles.groupCard} onPress={() => handleGroupPress(g.id)}>
              <View style={cgStyles.groupCardTop}>
                <View style={[cgStyles.groupIconWrap, { backgroundColor: colors.primary + "14" }]}>
                  <Users size={22} color={colors.primary} />
                </View>
                <View style={cgStyles.groupCardBody}>
                  <Text style={cgStyles.groupCardName}>{g.name}</Text>
                  <Text style={cgStyles.groupCardDesc} numberOfLines={2}>{g.description}</Text>
                </View>
                <ChevronRight size={16} color={colors.mutedForeground} />
              </View>
              <View style={cgStyles.groupCardMeta}>
                <View style={cgStyles.privacyBadge}>
                  {g.privacy === "Public"
                    ? <Globe size={10} color={colors.mutedForeground} />
                    : <Lock size={10} color={colors.mutedForeground} />}
                  <Text style={cgStyles.privacyBadgeText}>{g.privacy}</Text>
                </View>
                <Text style={cgStyles.memberCountText}>{g.memberCount} members</Text>
                <View style={cgStyles.spacer} />
                <View style={cgStyles.joinedPill}>
                  <CheckCircle size={10} color={colors.primary} />
                  <Text style={cgStyles.joinedPillText}>Joined</Text>
                </View>
              </View>
            </Pressable>
          ))}
      ...
      {discoverGroups.length > 0 && (
        <>
          <View style={cgStyles.sectionHeader}>
            ...
            <Text style={cgStyles.sectionTitle}>DISCOVER GROUPS</Text>
          </View>
          {discoverGroups.map((g) => (
            <View key={g.id} style={cgStyles.groupCard}>
              <View style={cgStyles.groupCardTop}>
                <View style={[cgStyles.groupIconWrap, { backgroundColor: g.privacy === "Private" ? colors.secondary : colors.primary + "14" }]}>
                  {g.privacy === "Private"
              ...
              <View style={cgStyles.groupCardMeta}>
                ...
                <View style={cgStyles.spacer} />
                {g.privacy === "Public" ? (
                  <Pressable style={cgStyles.joinBtn} onPress={() => handleJoin(g.id)}>
                    <Text style={cgStyles.joinBtnText}>Join</Text>
                  </Pressable>
                ) : (
                  <View style={cgStyles.invitePill}>
                    <Lock size={10} color={colors.mutedForeground} />
                    <Text style={cgStyles.invitePillText}>Invite only</Text>
                  </View>
                )}
              </View>
            </View>
      ...
      {groups.length === 0 && (
        <View style={cgStyles.emptyWrap}>
          <View style={[cgStyles.emptyIconWrap, { backgroundColor: colors.primary + "14" }]}>
            <Users size={30} color={colors.primary} />
          </View>
          <Text style={cgStyles.emptyTitle}>No groups yet</Text>
          <Text style={cgStyles.emptyDesc}>
            {isAdmin
              ? "Create a group to give your community a focused space for prayer and conversation."
              : "No groups have been created in this community yet."}
          </Text>
        </View>
      )}
    </ScrollView>
