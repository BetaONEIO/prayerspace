...
import { Home, Heart, MessageCircle, Globe, Users, Sparkles, Check } from "lucide-react-native";
...
export default function OnboardingFocus() {
  ...
  const [selected, setSelected] = useState<Set<string>>(new Set(["family"]));
    ...
    <SafeAreaView style={styles.container}>
      <View style={styles.inner}>
        ...
        <View style={styles.headingArea}>
          <Text style={styles.title}>Who do you pray for?</Text>
          <Text style={styles.subtitle}>We'll help you organize your contact list and requests.</Text>
        </View>
          ...
          {FOCUS_OPTIONS.map((opt) => {
            ...
            return (
              <Pressable key={opt.id} style={[styles.cell, isSelected && styles.cellSelected]} onPress={() => toggle(opt.id)} testID={`focus-${opt.id}`}>
                {isSelected && <View style={styles.checkBadge}><Check size={12} color={colors.primary} strokeWidth={3} /></View>}
                <IconComp size={36} color={isSelected ? colors.primary : colors.mutedForeground} />
...
function createStyles(colors: ThemeColors) {
  return StyleSheet.create({
    ...
    cell: { width: "47%", aspectRatio: 1, backgroundColor: colors.card, borderRadius: 24, borderWidth: 2, borderColor: colors.border, alignItems: "center", justifyContent: "center", gap: 12, position: "relative" },
    cellSelected: { borderColor: colors.primary, backgroundColor: colors.accent },
    checkBadge: { position: "absolute", top: 12, right: 12, width: 22, height: 22, borderRadius: 11, backgroundColor: colors.accent, alignItems: "center", justifyContent: "center" },
    cellLabel: { fontSize: 15, fontWeight: "700" as const, color: colors.mutedForeground },
